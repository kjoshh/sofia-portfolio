...
|---|---------|-------|-------|--------------|
| B13 ✅ | Kritisch | `js/index.js` | 729–752 | **Race Condition im Rain-Generator.** `_rainGen`-Counter soll mehrfache `startRain()`-Aufrufe verhindern, aber bei schnellen Interaktionen können Aufrufe überlappen. `if (_rainGen !== gen)`-Checks sind nicht synchron mit DOM-Operationen — es entstehen Physics-Bodies die nicht aufgeräumt werden. Fix: Mutex/Flag oder vorherige Animation komplett abbrechen. |
| B14 ✅ | Hoch | `js/index.js` | 731–752 | **setTimeout in Loop ohne Referenz.** Mehrere `setTimeout()`-Aufrufe in Schleife, aber kein Handle gespeichert. Bei Navigation feuern Timer auf möglicherweise entfernten DOM-Elementen. Fix: Handles in Array speichern, bei Cleanup clearen. |
...
| # | Schwere | Datei | Beschreibung |
|---|---------|-------|--------------|
| H6 | Niedrig | alle HTML | ✅ `defer` auf alle Script-Tags gesetzt. |
